...
bool RoundcubeBackup::loadConfiguration()
{
    ...
    const int leftFirstColonIdx = dsn.indexOf(QLatin1Char(':'));
    ...
    const int lastAtIdx = dsn.lastIndexOf(QLatin1Char('@'));

    const QString authority = dsn.mid(leftFirstColonIdx + 3, lastAtIdx - leftFirstColonIdx - 3);
    const int authColonIdx = authority.indexOf(QLatin1Char(':'));
    ...
    const int slashIdx = hostAndPath.indexOf(QLatin1Char('/'));
    const QString hostAndPort = hostAndPath.left(slashIdx);
    const QString name = hostAndPath.mid(slashIdx + 1);
    const int portColonIdx = hostAndPort.indexOf(QLatin1Char(':'));
    QString host;
    if (portColonIdx > -1) {
